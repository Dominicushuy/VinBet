  ...
  const shareResult = () => {
    if (navigator.share) {
      navigator.share({
        ...
        text: `Tôi vừa đặt cược ${formatCurrency(amount)} vào số ${chosenNumber}. Chơi cùng tôi!`,
        url: `/games/${gameId}`
      })
    }
  }
            ...
            <div className='p-6'>
              <div className='mb-4 space-y-3'>
                <div className='flex justify-between'>
                  <span className='text-sm text-muted-foreground'>Mã cược:</span>
                  <span className='font-medium'>#{betId.substring(0, 8)}</span>
                </div>

                <div className='flex justify-between'>
                  <span className='text-sm text-muted-foreground'>Số đã chọn:</span>
                  <span className='font-bold text-primary text-lg'>{chosenNumber}</span>
                </div>
                ...
                <div className='flex justify-between'>
                  <span className='text-sm text-muted-foreground'>Tiền thưởng tiềm năng:</span>
