// src/components/finance/TransactionChartView.tsx
"use client";
...
interface TransactionChartViewProps {
  filters: {
    type?: string;
    status?: string;
    startDate?: string;
...
export function TransactionChartView({ filters }: TransactionChartViewProps) {
  ...
  const generateMockData = (chartType: string, timeRange: string) => {
    ...
    if (active && payload && payload.length) {
      return (
        <div className="bg-background border p-2 rounded shadow-md">
          {chartType !== "pie" && (
            <p className="text-sm font-medium">{`Ngày: ${label}`}</p>
          )}
