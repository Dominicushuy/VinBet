// src/components/finance/WithdrawalFlowSteps.tsx
"use client";
...
interface WithdrawalFlowStepsProps {
  userBalance: number;
  config: {
    withdrawal_methods: {
      id: string;
...
export function WithdrawalFlowSteps({
  userBalance,
  config,
}: WithdrawalFlowStepsProps) {
  ...
  const handleStep1Submit = async (data: WithdrawalFormValues) => {
    // Get the selected payment method
    const selectedMethod = config.withdrawal_methods.find(
  ...
  const renderPaymentMethodFields = () => {
    ...
    const selectedMethod = config.withdrawal_methods.find(
      (method) => method.id === watchPaymentMethod
    );
          ...
          <CardHeader>
            <div className="flex justify-between items-center">
              <div>
                ...
                <CardDescription>
                  Rút tiền từ số dư VinBet về tài khoản của bạn
                </CardDescription>
              </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                          ...
                          <div className="relative">
                            ...
                            <Input
                              ...
                              onChange={(e) => {
                                ...
                                field.onChange(
                                  isNaN(value) ? config.min_withdrawal : value
                                );
                              }}
                            />
                          </div>
                        ...
                        <FormDescription>
                          Số tiền tối thiểu:{" "}
                          {formatMoney(config.min_withdrawal)}, tối đa:{" "}
                          {formatMoney(
                            Math.min(config.max_withdrawal, userBalance)
                          )}
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                  ...
                  {watchAmount > 0 && (
                    <div className="bg-muted/50 p-3 rounded-md space-y-2">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="text-sm font-medium">
                          {formatMoney(calculateFee(watchAmount))}
                        </span>
                      </div>
                      <div className="border-t pt-2 mt-2 flex justify-between">
                        <span className="font-medium">Số tiền nhận được:</span>
                        <span className="font-medium text-primary">
                          {formatMoney(calculateNetAmount(watchAmount))}
                        </span>
                      </div>
                    </div>
                  )}
              ...
              <div className="space-y-6 text-center py-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-muted-foreground">
                    Yêu cầu rút tiền của bạn đã được gửi thành công. Vui lòng
                    chờ admin xác nhận và tiền sẽ được chuyển vào tài khoản của
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        {
                          config.withdrawal_methods.find(
                            (m) => m.id === watchPaymentMethod
                          )?.name
                        }
                      </span>
                    </div>
                ...
                <Alert
                  variant="default"
                  className="bg-blue-50 text-blue-800 border-blue-200 text-left"
                >
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm">
                  Chọn phương thức và nhập số tiền muốn rút
                </p>
              </div>
              <div className="flex items-center gap-2">
                <div className="flex-shrink-0 h-6 w-6 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-medium text-sm">
              ...
              <div className="flex items-center gap-2">
                ...
                <p className="text-sm">
                  Xác nhận thông tin và gửi yêu cầu rút tiền
                </p>
              </div>
              <div className="flex items-center gap-2">
                <div className="flex-shrink-0 h-6 w-6 rounded-full bg-primary flex items-center justify-center text-primary-foreground font-medium text-sm">
                ...
                <p className="text-sm">
                  Kiểm tra trạng thái yêu cầu trong lịch sử rút tiền
                </p>
              </div>
            </div>
            ...
            <div className="pt-4 border-t">
              ...
              <ul className="text-xs text-muted-foreground space-y-1">
                <li>
                  • Thông tin tài khoản nhận tiền phải trùng khớp với tên tài
                  khoản VinBet
                </li>
                <li>
                  • Số tiền tối thiểu cho mỗi lần rút là{" "}
                  {formatMoney(config.min_withdrawal)}
                </li>
                <li>• Bạn chỉ có thể rút tiền về tài khoản của chính mình</li>
                <li>
                  • Không thể hủy yêu cầu rút tiền sau khi đã được xác nhận
                </li>
