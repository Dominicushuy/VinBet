// src/components/finance/FinancialOverviewChart.tsx
"use client";
...
interface FinancialOverviewChartProps {
  ...
  filter: "all" | "deposit" | "withdrawal" | "bet" | "win";
}
...
export function FinancialOverviewChart({
  ...
  // Mô phỏng dữ liệu trong trường hợp không có API hoặc đang loading
...
function generateMockData(period: string, filter: string) {
  const data = [];
  let days = 30;
  ...
  switch (period) {
    ...
    case "month":
      ...
      break;
    case "year":
      days = 12; // Sử dụng tháng thay vì ngày cho year view
      break;
  }
  ...
  for (let i = 0; i < days; i++) {
    ...
    if (period === "year") {
      // Nếu là year view, lùi theo tháng
      ...
      if (filter === "all" || filter === "deposit")
        entry.deposit = Math.floor(Math.random() * 5000000) + 1000000;
      if (filter === "all" || filter === "withdrawal")
        entry.withdrawal = Math.floor(Math.random() * 3000000) + 500000;
      if (filter === "all" || filter === "bet")
        entry.bet = Math.floor(Math.random() * 4000000) + 2000000;
      if (filter === "all" || filter === "win")
        entry.win = Math.floor(Math.random() * 7000000) + 1000000;

      data.push(entry);
    } else {
      // Nếu là week hoặc month view, lùi theo ngày
      ...
      if (filter === "all" || filter === "deposit")
        entry.deposit = Math.floor(Math.random() * 2000000) + 500000;
      if (filter === "all" || filter === "withdrawal")
        entry.withdrawal = Math.floor(Math.random() * 1000000) + 200000;
      if (filter === "all" || filter === "bet")
        entry.bet = Math.floor(Math.random() * 1500000) + 500000;
      if (filter === "all" || filter === "win")
        entry.win = Math.floor(Math.random() * 3000000) + 300000;
