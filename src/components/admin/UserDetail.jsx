...
const safeFormatDate = (dateStr, dateFormat) => {
          ...
          <CardContent>
            ...
            <div className='space-y-4'>
              ...
              <div className='flex justify-between text-sm'>
                ...
                <span>{safeFormatDate(user.created_at, 'HH:mm - dd/MM/yyyy')}</span>
              </div>
              <div className='flex justify-between text-sm'>
                <span className='text-muted-foreground'>Đăng nhập cuối:</span>
                <span>
                  {stats.last_login ? safeFormatDate(stats.last_login, 'HH:mm - dd/MM/yyyy') : 'Không có dữ liệu'}
                </span>
              </div>
              <div className='flex justify-between text-sm'>
          ...
          <CardContent>
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value='overview' className='space-y-4 pt-4'>
                <div className='grid gap-4 grid-cols-2'>
                  <Card>
                    ...
                    <CardContent>
                      ...
                      <p className='text-xs text-muted-foreground'>Nạp: {formatCurrency(stats.total_deposits || 0)}</p>
                  ...
                  <Card>
                    ...
                    <CardContent>
                      <div className='text-2xl font-bold'>{formatCurrency(stats.net_gambling || 0)}</div>
                      <p className='text-xs text-muted-foreground'>
                ...
                <Card>
                  <CardHeader className='pb-2'>
                    <CardTitle className='text-sm font-medium'>Thống kê cược</CardTitle>
                  </CardHeader>
                          ...
                          {recentTransactions.slice(0, 3).map(tx => (
                            <div key={tx.id} className='flex items-center justify-between'>
                              <div>
                                ...
                                <p className='text-xs text-muted-foreground'>
                                  {safeFormatDate(tx.created_at, 'HH:mm - dd/MM/yyyy')}
                                </p>
                              </div>
                          ...
                          {recentBets.slice(0, 3).map(bet => (
                            <div key={bet.id} className='flex items-center justify-between'>
                              <div>
                                ...
                                <p className='text-xs text-muted-foreground'>
                                  {safeFormatDate(bet.created_at, 'HH:mm - dd/MM/yyyy')}
                                </p>
                              </div>
                              <p
              ...
              <TabsContent value='bets' className='pt-4'>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {recentBets.length === 0 ? (
                        ...
                        recentBets.map(bet => (
                          <TableRow key={bet.id}>
                            <TableCell className='font-mono text-xs'>{bet.id.substring(0, 8)}...</TableCell>
                            <TableCell className='font-mono text-xs'>{bet.game_round_id.substring(0, 8)}...</TableCell>
                            ...
                            <TableCell>
                              <Badge
                                variant={
                                  bet.status === 'won' ? 'default' : bet.status === 'lost' ? 'destructive' : 'outline'
                                }
                              >
                                {bet.status === 'won'
                                  ? 'Thắng'
                            ...
                            <TableCell>{safeFormatDate(bet.created_at, 'HH:mm - dd/MM')}</TableCell>
                          </TableRow>
                        ))
                      )}
                    </TableBody>
                ...
                <div className='mt-2 text-right'>
                  <Button variant='link' onClick={() => router.push(`/admin/users/${userId}/bets`)}>
                    Xem tất cả
                  </Button>
                </div>
              </TabsContent>
              ...
              <TabsContent value='transactions' className='pt-4'>
                      ...
                      {recentTransactions.length === 0 ? (
                        ...
                        recentTransactions.map(tx => (
                          <TableRow key={tx.id}>
                            ...
                            <TableCell>{safeFormatDate(tx.created_at, 'HH:mm - dd/MM')}</TableCell>
                          </TableRow>
                        ))
                      )}
                    </TableBody>
                ...
                <div className='mt-2 text-right'>
                  <Button variant='link' onClick={() => router.push(`/admin/users/${userId}/transactions`)}>
                    Xem tất cả
                  </Button>
                </div>
              </TabsContent>
