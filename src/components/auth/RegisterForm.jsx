...
import { Eye, EyeOff, Loader2, ArrowLeft, ArrowRight, CheckCircle, X, Shield } from 'lucide-react'
...
export function RegisterForm() {
  ...
  async function onSubmit(data) {
    ...
    if (step === 2) {
      const passwordResult = await form.trigger(['password', 'confirmPassword'])
                  ...
                  {field.value && (
                    <div className='space-y-2'>
                      <div className='flex justify-between items-center'>
                        <span className='text-xs'>Độ mạnh: {getPasswordStrengthText()}</span>
                      ...
                      <div className='flex flex-wrap gap-1 mt-2'>
                        <PasswordRequirement text='6 ký tự' satisfied={field.value.length >= 6} />
                        <PasswordRequirement text='Chữ hoa' satisfied={/[A-Z]/.test(field.value)} />
                        <PasswordRequirement text='Số' satisfied={/[0-9]/.test(field.value)} />
                      </div>
                    </div>
                  )}
        ...
        return (
          <>
            <FormField
              ...
              render={({ field }) => (
                <FormItem className='space-y-3'>
                  <FormLabel className='text-base'>Mã giới thiệu (không bắt buộc)</FormLabel>
                  <FormControl>
                    <Input
                ...
                <FormItem className='space-y-3'>
                  <div className='flex items-start space-x-3 pt-2'>
                    ...
                    <div className='space-y-1 leading-none'>
                      ...
                      <FormDescription className='text-xs'>Bạn có thể hủy đăng ký bất cứ lúc nào.</FormDescription>
                    </div>
                  </div>
                  <FormMessage />
                </FormItem>
    ...
    <div className='space-y-6'>
      ...
      <Card className='border-muted'>
        ...
        <CardFooter className='flex justify-between border-t p-4'>
          <Button type='button' variant='ghost' onClick={handlePrevStep} disabled={step === 1 || isLoading}>
  ...
  return (
    <div
      className={`text-xs px-2 py-1 rounded-full flex items-center gap-1 ${
        satisfied ? 'bg-success/20 text-success' : 'bg-muted text-muted-foreground'
      }`}
    >
      {satisfied ? <CheckCircle className='h-3 w-3' /> : <Shield className='h-3 w-3' />}
      {text}
    </div>
  )
}
