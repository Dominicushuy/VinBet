// src/components/game/WinnerList.tsx
"use client";
...
export function WinnerList({ gameRound }: WinnerListProps) {
  ...
  if (winners.length === 0) {
    return (
      <Card>
        ...
        <CardContent className="flex flex-col items-center justify-center gap-4 py-6">
          <p className="text-center text-muted-foreground">
            Không có người thắng cuộc trong lượt chơi này.
          </p>
        </CardContent>
      </Card>
    );
            ...
            <TableBody>
              {displayWinners.map((winner) => (
                <TableRow key={winner.id}>
                  ...
                  <TableCell>
                    {format(
                      new Date(winner.created_at || new Date()),
                      "HH:mm, dd/MM"
                    )}
                  </TableCell>
