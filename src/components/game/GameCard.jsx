...
import { Clock, Trophy, Users, ExternalLink, Timer, Ban, Calendar } from 'lucide-react'
  ...
  useEffect(() => {
    const updateTimeAndProgress = () => {
      ...
      if (now >= startTime && now < endTime) {
        ...
        // Trò chơi đã kết thúc hoặc bị hủy
        setIsLive(false)
    ...
    >
        ...
        <div className='relative h-40 bg-gradient-to-br from-primary/10 to-accent/5 overflow-hidden'>
          ...
          <div className='absolute inset-0 overflow-hidden'>
            <div
              ...
              } -top-20 -right-20 opacity-30`}
            ></div>
            <div
              className={`absolute h-32 w-32 rounded-full ${
                isLive ? 'bg-green-500/10' : 'bg-blue-500/5'
              } bottom-10 left-10 opacity-20`}
            ></div>
          </div>
          ...
          {/* Game Result (nếu đã hoàn thành) */}
          {game.status === 'completed' && game.result && (
            <div className='absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-[1px]'>
              <div className='bg-white/10 backdrop-blur-md rounded-lg p-3 text-center'>
                <div className='text-xs font-medium text-white/80 mb-1'>Kết quả</div>
                <div className='text-3xl font-bold text-white'>{game.result}</div>
              </div>
            </div>
          )}
      ...
      <CardContent className='p-4 space-y-3'>
        ...
        <div className='flex items-center text-sm gap-1 text-muted-foreground'>
          ...
          <span className={cn('font-medium', isLive ? 'text-green-600 dark:text-green-400' : '')}>{timeLeft}</span>
        </div>
