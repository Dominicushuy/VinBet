...
export async function GET(request) {
  try {
    ...
    if (chartType === 'pie') {
      ...
      // Execute queries
      const [depositResult, withdrawalResult, betResult, winResult, referralResult] = await Promise.all([
        depositQuery,
        withdrawalQuery,
        betQuery,
        winQuery,
      ...
      // Calculate totals
      const depositTotal = (depositResult.data || []).reduce((sum, item) => sum + item.amount, 0)
      const withdrawalTotal = (withdrawalResult.data || []).reduce((sum, item) => sum + item.amount, 0)
      const betTotal = (betResult.data || []).reduce((sum, item) => sum + item.amount, 0)
      const winTotal = (winResult.data || []).reduce((sum, item) => sum + item.amount, 0)
      const referralTotal = (referralResult.data || []).reduce((sum, item) => sum + item.amount, 0)
