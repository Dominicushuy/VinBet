    ...
    const exportData =
      transactions?.map((transaction) => {
        return {
          ...
          "Số tiền": formatAmountWithPrefix(
            transaction.amount,
            transaction.type
          ),
...
function formatAmountWithPrefix(amount: number, type: string) {
  const isPositive =
    type === "deposit" || type === "win" || type === "referral_reward";
...
async function exportToCSV(data: any[]) {
  ...
  return new NextResponse(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="vinbet-giao-dich-${format(
        new Date(),
        "dd-MM-yyyy",
        { locale: vi }
      )}.csv"`,
...
// Export to Excel
async function exportToExcel(data: any[], userProfile: any) {
  // Create worksheet
...
// Export to PDF
async function exportToPDF(data: any[], userProfile: any) {
  // Create PDF document
  ...
  // Add footer
  page.drawText(
    `Trang ${pdfDoc.getPageCount()} / ${Math.ceil(data.length / 15)}`,
    {
      x: 750,
      y: 30,
      size: fontSize,
