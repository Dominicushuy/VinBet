...
export async function POST(request) {
  try {
    ...
    await supabase.rpc('create_notification', {
      ...
      p_content: 'Tài khoản của bạn đã được kết nối với Telegram. Bạn sẽ nhận được thông báo quan trọng qua Telegram.',
      p_type: 'system'
    })
    ...
    return NextResponse.json({ error: 'Không thể kết nối với Telegram' }, { status: 500 })
  }
}
...
export async function DELETE(request) {
  try {
    ...
    return NextResponse.json({ error: 'Không thể ngắt kết nối với Telegram' }, { status: 500 })
  }
}
...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase.from('profiles').select('telegram_id').eq('id', userId).single()
    ...
    return NextResponse.json({ error: 'Không thể kiểm tra trạng thái Telegram' }, { status: 500 })
  }
}
