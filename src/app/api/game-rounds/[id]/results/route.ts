...
export async function GET(
  ...
  try {
    ...
    if (
      ...
      !(await isUserAdmin(supabase, sessionData.session.user.id))
    ) {
      return NextResponse.json(
        { error: "Game round results not available yet" },
        { status: 403 }
      );
    }

    // Lấy thống kê cược
    const { data: betStats, error: betStatsError } = await supabase.rpc(
